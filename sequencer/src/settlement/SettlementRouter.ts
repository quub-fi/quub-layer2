...
export class SettlementRouter {
  ...
  registerAdapter(adapter: ChainAdapter): void {
    ...
    console.log(
      `[Settlement Router] Registered ${adapter.config.name} adapter`
    );
  }
  ...
  async connect(): Promise<void> {
    ...
    console.log(
      `[Settlement Router] Connected to ${connectedCount}/${this.adapters.size} chains`
    );
  }
  ...
  async analyzeBatch(batch: BatchSubmission): Promise<SettlementDecision[]> {
    console.log(
      `[Settlement Router] Analyzing batch ${batch.batchIndex} across all chains...`
    );
    ...
    for (const [chainId, adapter] of this.adapters) {
      if (!adapter.isConnected()) {
        console.log(
          `[Settlement Router] Skipping ${adapter.config.name} (not connected)`
        );
        continue;
      }
      ...
      try {
        ...
        console.error(
          `[Settlement Router] Failed to analyze ${adapter.config.name}:`,
          error
        );
      }
    }
    ...
    decisions.forEach((d, i) => {
      ...
      console.log(
        `  ${d.chainName}: score ${d.score.toFixed(2)}, cost ${this.formatCost(
          d.estimatedCost
        )}, time ${d.estimatedTime}s ${marker}`
      );
    });
  ...
  async submitToChain(
    ...
    console.log(
      `[Settlement Router] Submitting batch ${batch.batchIndex} to ${adapter.config.name}`
    );
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.error(
          `[Settlement Router] Attempt ${attempt + 1}/${
            maxRetries + 1
          } failed:`,
          error
        );
    ...
    throw new Error(
      `Failed to submit batch after ${maxRetries + 1} attempts: ${
        lastError?.message
      }`
    );
  }

  async submitBatchWithFailover(
    batch: BatchSubmission
  ): Promise<SettlementResult> {
    ...
    for (const decision of decisions) {
      try {
        console.log(
          `[Settlement Router] Attempting settlement on ${decision.chainName}...`
        );
        return await this.submitToChain(batch, decision.chainId, 0); // No retries, failover instead
      } catch (error) {
        console.error(
          `[Settlement Router] ${decision.chainName} failed, trying next chain...`
        );
      }
    }
  ...
  private updateStats(chainId: ChainId, result: SettlementResult): void {
    ...
    this.stats.averageCost =
      this.stats.totalCost / BigInt(this.stats.totalBatches);

    // Update average time (running average)
    this.stats.averageTime =
      (this.stats.averageTime * (this.stats.totalBatches - 1) +
