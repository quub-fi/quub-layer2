...
import { BaseChainAdapter, ChainId, ChainConfig, BatchSubmission, SettlementResult } from './IChainAdapter';
  ...
  async submitBatch(batch: BatchSubmission): Promise<SettlementResult> {
    ...
    try {
      ...
      const batchData = ethers.AbiCoder.defaultAbiCoder().encode(
        ...
        [batch.transactions.map(tx => ethers.keccak256(tx)), batch.stateRoot, batch.timestamp]
      );
      
      // Submit transaction
      console.log(`[${this.config.name}] Submitting batch ${batch.batchIndex}...`);
      ...
      console.log(`[${this.config.name}] Batch ${batch.batchIndex} confirmed in ${finalizationTime}ms, cost: ${ethers.formatEther(cost)} ETH`);
